...
def draw_names(canvas, name_data, lookup_names):
    ...
    # ----- Write your code below this line ----- #
    y_interval = (CANVAS_HEIGHT - GRAPH_MARGIN_SIZE * 2) / MAX_RANK
    ...
    for i in range(len(lookup_names)):
        ...
        for j in range(len(YEARS)):
            ...
            if year in name_data[lookup_name]:
                ...
                y = GRAPH_MARGIN_SIZE + y_interval * rank
                canvas.create_text(get_x_coordinate(CANVAS_WIDTH, j) + TEXT_DX, y,
                                   text=lookup_name+" "+str(rank), fill=COLORS[i % len(COLORS)], anchor=tkinter.SW)
            else:
                y = CANVAS_HEIGHT - GRAPH_MARGIN_SIZE
                canvas.create_text(get_x_coordinate(CANVAS_WIDTH, j) + TEXT_DX, y, text=str(lookup_name)+" *",
                                   fill=COLORS[i % len(COLORS)], anchor=tkinter.SW)

            if j != len(YEARS) - 1:
                next_year = str(YEARS[j + 1])

                if next_year in name_data[lookup_name]:
                    next_rank = int(name_data[lookup_name][next_year])
                    next_y = GRAPH_MARGIN_SIZE + y_interval * next_rank
                else:
                    next_y = CANVAS_HEIGHT - GRAPH_MARGIN_SIZE
                canvas.create_line(get_x_coordinate(CANVAS_WIDTH, j), y, get_x_coordinate(CANVAS_WIDTH, j + 1), next_y,
